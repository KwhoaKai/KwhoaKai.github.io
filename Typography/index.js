const letters = function(p) {
  ...
  p.setup = function() {
    ...
    p.infoMap = {
      k: {
        term: ["Ascender", "Leg"],
        info: [
          "The portion of the stem that projects above the median.",
          "Short strokes off the stem of the letterform, inclined downward."
        ]
      },
      g: {
        term: ["Ear", "Loop"],
        info: [
          "The stroke extending out from the main body of the letterform.",
      ...
      s: {
        term: ["Stem"],
        info: ["The curved stem of the letter s"]
      }
    };
    ...
    p.tip = new Tips(p.width * 0.33, p.height * 0.53, "try shaking a letter");
    p.tip.fadeTo(230);
    p.bar = new Bar();
  ...
  class Bar {
    constructor() {
      this.init = -1;
      this.h = p.height / 10;
      this.finalH = parent.offsetHeight - 6;
      this.fade = 150;
      this.finalFade = 0;
      this.initiate = false;
    }
    ...
    display() {
      this.fade = this.initiate
        ? p.lerp(this.fade, this.finalFade, 0.2)
        : this.fade;
      ...
      if (this.init == 0 && this.finalH % this.h < 0.05) {
        ...
        p.tip.changeLoc(p.width * 0.33, p.height * 0.53);
        p.tip.fadeTo(230);
      }
      p.pop();
    }
  ...
  class Letter {
    ...
    dragged() {
      let distM = Math.sqrt(
        Math.pow(this.x - p.mouseX, 2) + Math.pow(this.y - p.mouseY, 2)
      );
    ...
    display(mx, my) {
      ...
      let distM = Math.sqrt(
        Math.pow(this.x - mx, 2) + Math.pow(this.y - my, 2)
      );
      let col = distM <= p.width / 20 ? true : false;
      ...
      if (col && this.startFade && (!p.noHover || this.isHover)) {
        ...
        !col &&
        ((this.finishedFade && this.blink == false) || this.isHover)
      ) {
        this.fade = 255;
        if (this.isHover) {
          this.isHover = false;
      ...
      if (this.y > p.height * 0.97 || this.y < p.height * 0.03) this.yDir *= -1;
  ...
  class InfoLetter {
    constructor(key, x, y, retCoords) {
      ...
      this.lineFin = this.multRet
        ? [
            [p.width * 0.65, p.height * 0.25],
            [p.width * 0.65, p.height * 0.65]
          ]
        : [p.width * 0.65, p.height / 2];
    }
    ...
    drawInfo() {
      this.fade2 = p.lerp(this.fade2, this.fullFade, 0.1);
      this.terms.forEach(function(term, i) {
        if (!this.multRet) {
          ...
          p.text(descrp, p.width * 0.68, p.height * 0.4);
          ...
          let descrp = this.terms[i];
          let info = this.info[i];
          let yTitle = this.lineFin[i][1] - p.height * 0.05;
          let yInfo = yTitle + p.height * 0.03;

          p.push();
          p.fill(255, 0, 0, this.fade);
          p.textSize(p.height * 0.1);
          p.text(descrp, p.width * 0.68, yTitle);
          p.fill(0, 0, 0, this.fade2);
          p.textSize(p.height * 0.06);
          p.rectMode(p.CORNER);
          p.text(info, p.width * 0.68, yInfo, p.width * 0.3, p.height);
          p.pop();
        }
      }, this);
    }
    ...
    drawQuote() {
      ...
      p.rectMode(p.CORNER);
      p.textAlign(p.CENTER);
      p.text(quote, p.width * 0.15, p.height * 0.25, p.width * 0.25, p.height);
      p.pop();
    }
    ...
    display() {
      if (this.centered) {
        ...
        if (this.multRet) {
          ...
          this.retCoords.forEach(coords =>
            p.line(coords[0], coords[1], p.width * 0.65, p.height / 2)
          );
        }
        ...
        p.ellipseMode(p.RADIUS);
        this.retCoords.forEach(coords =>
          p.ellipse(coords[0], coords[1], p.height * 0.08)
        );
        p.pop();
      ...
      if (
        Math.abs(p.width / 2 - this.x) < 0.3 &&
        Math.abs(p.height / 2 - this.y < 0.3)
      ) {
        this.centered = true;
      }

      p.push();
      this.y = p.lerp(this.y, p.height / 2, 0.2);
      this.x = p.lerp(this.x, p.width / 2, 0.2);
      p.rectMode(p.CENTER);
      p.noStroke();
      p.fill(0, 0, 0);
      ...
      if (this.centered)
        this.retCoords.forEach(coords => this.reticle(coords[0], coords[1]));
    }
  }
    ...
    display() {
      this.rad =
        this.finalRad - this.rad < 0.05
          ? p.width
          : p.lerp(this.rad, this.finalRad, 0.1);

      this.fill =
        this.finalFill - this.fill < 0.05
          ? this.finalFill
          : p.lerp(this.fill, this.finalFill, 0.1);
