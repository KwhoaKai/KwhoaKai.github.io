...
var letters = function letters(p) {
  var parent = document.getElementById("sketch");
  var popBox;
  var infoUp;
  var transMap;
  var bar;
  ...
  p.setup = function () {
    ...
    p.transMap = {
      // [[Info Letter], [Floater]]
      k: [[p.width / 17, p.height / 30], [p.width / 45, p.width / 10]],
      g: [[p.width / 23, -p.height / 11], [p.width / 45, p.width / 12]],
      t: [[p.width / 23, 0], [p.width / 45, p.width / 10.5]],
      v: [[p.width / 23, 0], [p.width / 45, p.width / 10.5]],
      x: [[p.width / 23, -p.height / 50], [p.width / 45, p.width / 10.5]],
      i: [[p.width / 23, p.height / 50], [p.width / 45, p.width / 9]],
      y: [[p.width / 23, -p.height / 20], [p.width / 45, p.width / 12]]
    };
    p.retMap = {
      k: [[p.width * 0.48, p.height * 0.3], [p.width * 0.54, p.height * 0.6]],
      g: [[p.width * 0.56, p.height * 0.32], [p.width * 0.5, p.height * 0.6]],
      t: [[p.width * 0.51, p.height * 0.4]],
      v: [[p.width * 0.5, p.height * 0.63]],
      x: [[p.width * 0.503, p.height * 0.408]],
      i: [[p.width * 0.505, p.height * 0.66]],
      y: [[p.width * 0.47, p.height * 0.7]]
    };
    p.infoMap = {
      k: {
        term: ["Ascender", "Leg"],
    ...
    p.smalls.forEach(function (key) {
      return p.floaters.push(new Letter(key));
    });
  };
  ...
  p.draw = function () {
    ...
    p.cursor.display();
    p.floaters.forEach(function (letter) {
      return letter.display(p.mouseX, p.mouseY);
    });
  ...
  function () {
    ...
    _createClass(Bar, [{
      ...
      value: function display() {
        this.h = this.init == -1 ? p.height / 9 : this.init == 0 ? p.lerp(this.h, this.finalH, 0.25) : this.finalH;
        p.push();
        p.rectMode(p.CENTER);
        p.noFill();
        p.stroke(0, 0, 0);
        ...
        if (this.init == 0 && this.finalH % this.h < 0.05) {
          this.init = 1;
          p.floaters.forEach(function (floater) {
            return floater.fadeIn();
          });
  ...
  function () {
    ...
    _createClass(Tips, [{
      key: "changeTip",
      value: function changeTip(tip) {
        this.tip = tip;
      }
    }, {
      key: "fadeTo",
      value: function fadeTo(finish) {
        this.finalFade = finish;
      }
    }, {
      key: "changeLoc",
      value: function changeLoc(x, y) {
        this.x = x;
        this.y = y;
      }
    }, {
  ...
  function () {
    ...
    _createClass(Letter, [{
      ...
      value: function dragged() {
        var distM = Math.sqrt(Math.pow(this.x - p.mouseX, 2) + Math.pow(this.y - p.mouseY, 2));
        var col = distM <= p.width / 20 ? true : false;
      ...
      key: "resetCoord",
      value: function resetCoord(x, y) {
        this.x = x < 0 ? 1 : x > p.width ? p.width - 1 : x;
        this.y = y > p.height * .97 ? p.height * .97 - 1 : y < 0 ? p.height * .03 + 1 : y;
      }
    }, {
      key: "display",
      value: function display(mx, my) {
        // Detect shake and opacity
        var distM = Math.sqrt(Math.pow(this.x - mx, 2) + Math.pow(this.y - my, 2));
        var col = distM <= p.width / 20 ? true : false;
        ...
        if (this.startFade && !this.finishFade) {
          this.fade = this.finalFade - this.fade < 5 ? 255 : p.lerp(this.fade, this.finalFade, 0.1);
          ...
          var diff = Math.abs(this.fade - this.finalFade) < 2;

          if (diff) {
            this.finalFade = this.finalFade == 255 ? 140 : 255;
          }
        } // Handle hover
  ...
  /*#__PURE__*/
  function () {
    function InfoLetter(key, x, y, retCoords) {
      _classCallCheck(this, InfoLetter);
      ...
      this.lineFin = this.multRet ? [[p.width * .65, p.height * .25], [p.width * .65, p.height * .65]] : [p.width * .65, p.height / 2];
    }

    _createClass(InfoLetter, [{
      key: "reticle",
      value: function reticle(x, y) {
      ...
      value: function drawInfo() {
        ...
        this.terms.forEach(function (term, i) {
          if (!this.multRet) {
            ...
            var _descrp = this.terms[i];
            var _info = this.info[i];
            var yTitle = this.lineFin[i][1] - p.height * .05;
            var yInfo = yTitle + p.height * .03;
            p.push();
            p.fill(255, 0, 0, this.fade);
            p.textSize(p.height * 0.1);
            p.text(_descrp, p.width * .68, yTitle);
            p.fill(0, 0, 0, this.fade2);
            p.textSize(p.height * .06);
            p.rectMode(p.CORNER);
            p.text(_info, p.width * .68, yInfo, p.width * .3, p.height);
            p.pop();
          }
        }, this);
      }
      ...
      key: "drawQuote",
      value: function drawQuote() {
        var quote = p.quoteMap[this.key];
        /*
        p.rectMode(p.CORNERS);
        p.noStroke();
        p.fill(255,255,255);
      ...
      value: function display() {
        ...
        if (this.centered) {
          ...
          if (this.multRet) {
            this.retCoords.forEach(function (coords, i) {
              return p.line(coords[0], coords[1], _this.lineFin[i][0], _this.lineFin[i][1]);
            }, this);
          } else {
            this.retCoords.forEach(function (coords) {
              return p.line(coords[0], coords[1], p.width * .65, p.height / 2);
            });
          }
          ...
          p.ellipseMode(p.RADIUS);
          this.retCoords.forEach(function (coords) {
            return p.ellipse(coords[0], coords[1], p.height * .08);
          });
          p.pop();
          this.drawInfo();
          this.drawQuote();
        ...
        p.text(this.key, this.transX, this.transY, p.width, p.height * 0.8);
        p.pop();
        if (this.centered) this.retCoords.forEach(function (coords) {
          return _this.reticle(coords[0], coords[1]);
        });
      }
    }]);
  ...
  function () {
    ...
    _createClass(PopBox, [{
      key: "display",
      value: function display() {
        this.rad = this.finalRad - this.rad < .05 ? p.width : p.lerp(this.rad, this.finalRad, .1);
        this.fill = this.finalFill - this.fill < .05 ? this.finalFill : p.lerp(this.fill, this.finalFill, .1);
        p.push();
...
var letterp5 = new p5(letters, "sketch");